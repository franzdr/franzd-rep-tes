...
public class Slave {
   ...
   private TpccBenchmarkStage stressorHook;
            ...
            if (key.isReadable()) {
               ...
               if (byteBuffer.position() == expectedSize + 4) {
                  if(steadyState==false){
                    DistStage stagek = null;
                    stagek = (DistStage) SerializationHelper.deserialize(byteBuffer.array(), 4, expectedSize);
                    if(stagek instanceof TpccBenchmarkStage){
                        steadyState = true;
                    }
                  Runnable runnable = new Runnable() {
                     public void run() {
                        try {
                           ...
                           if(stage instanceof TpccBenchmarkStage){
                               stressorHook = (TpccBenchmarkStage)stage;
                               byteBuffer.clear();
                           }
               ...
               }else{//ricevuta nuova configurazione dei parametri di controllo
                      log.info("Ricevuta nuova configurazione dal master.");
                      DistStage stage = (DistStage) SerializationHelper.deserialize(byteBuffer.array(), 4, expectedSize);
                      TpccStressorAlternative stressor = stressorHook.getTpccStressorA();
                      if(stressor==null)
                          log.info("TpccStressor has not been intialized yet.");
                      else{
                          stressor.blockTransactionRequests();
                          stressor.updateConfig((TpccBenchmarkStage)stage);
                          stressor.unBlockTransactionRequests();
                      }
                          byteBuffer.clear();
                  }
                } 
