...
public class LaunchMaster {
   ...
   public static void main(String[] args) {
      ...
      try {
         ...
         setMLOnMasterConfig(args, masterConfig);
   ...
   private static void setMLOnMasterConfig(String[] args, MasterConfig masterConfig){
       String filename = null;
       for (int i = 0; i < args.length - 1; i++) {
         if (args[i].equals("-network")) {
            filename = args[i + 1];
            masterConfig.setMLNetworkMode(filename);
            break;
         }
         if (args[i].equals("-training")) {
            filename = args[i + 1];
            masterConfig.setMLTrainingMode(filename);
            break;
         }
      }
      ...
      if(filename == null){
          log.warn("No file specified for the current Machine Learning Mode.");
      }
   }
